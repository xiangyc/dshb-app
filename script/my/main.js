...
function loadAdvInfo(){
	api.ajax({
		...
		dataType : 'json',
		returnAll : false,
		url : global.getRequestUri() + '/member/adv/total',
		headers : global.getRequestToken()
	}, function(ret, err) {
		if(ret){
			$api.addCls($api.byId("i-1"), "aui-hide");
			$api.addCls($api.byId("i0"), "aui-hide");
			$api.addCls($api.byId("i1"), "aui-hide");
			$api.addCls($api.byId("i2"), "aui-hide");
			for(var i=0; i<ret.length; i++){
				$api.removeCls($api.byId("i" + ret[i].status), "aui-hide");
				$api.html($api.byId("i" + ret[i].status), ret[i].total);
			}
			$api.addCls($api.byId("i2"), "aui-hide");
		}
	});
